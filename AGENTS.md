...
> - **MUST Wait for Explicit Authorization**: Do not take action unless explicitly told to do so. If the user highlights a piece of code, brings up a bug, or brings something to your attention, your ONLY job is to analyze it, investigate the root cause, and explain what is wrong. **Do not write the fix, delete files, or execute changes unless the user explicitly gives you the order to fix it.** Wait for clear authorization before taking action.
